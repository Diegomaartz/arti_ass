...
\PassOptionsToClass{a4paper}{article} % Force A4 paper

\documentclass{svproc} % Springer class

% Add AFTER documentclass
\setlength{\headsep}{12pt} % Set headsep to 12pt

\usepackage{amsmath, amssymb}
\usepackage{booktabs}  % Must add to preamble for rules
%\usepackage{authblk}
...
\hypersetup{
	...
	pdfborder  = {0 0 0},    % Remove visible border completely
}
\usepackage{natbib} % For citation sorting
\bibliographystyle{unsrt} % References in citation order
...
\titlerunning{VitaEX: CV Analysis and Job Recommendations}
\author{
...
\begin{document}
	...
	\begin{table}[ht]
		\caption{Comparison of CV analysis and job recommendation systems}
		\label{tab:related}
		\centering
		\small
		...
		\begin{tabular}{lccccc}
			\hline
			\textbf{Systems} & 
			\begin{tabular}{c}\textbf{Recomendations}\\ \textbf{based on job offers}\end{tabular} & 
			\begin{tabular}{c}\textbf{CV}\\ \textbf{temps.}\end{tabular} & 
			\textbf{ML} & 
			\begin{tabular}{c}\textbf{Job offer}\\ \textbf{recommendation}\end{tabular} & 
			\begin{tabular}{c}\textbf{Free for}\\ \textbf{students}\end{tabular} \\
			\hline
			NLU for CV~\cite{estadoarte1} & $\checkmark$ & & & & \\
			\hline
			GIRL~\cite{estadoarte2} & $\checkmark$ & & $\checkmark$ & & \\
			\hline
			ResumeNet~\cite{estadoarte3} & & & $\checkmark$ & & \\
			\hline
			NN Recruitment~\cite{estadoarte4} & & & $\checkmark$ & & \\
			\hline
			LinkedIn Builder~\cite{estadoarte5} & $\checkmark$ & $\checkmark$ & & & $\checkmark$ \\
			\hline
			CVapp~\cite{estadoarte8} & & $\checkmark$ & & & \\
			\hline
			Jobania~\cite{estadoarte6} & $\checkmark$ & $\checkmark$ & & & \\
			\hline
			CVMATCHER~\cite{estadoarte7} & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & \\
			\hline
			\textbf{VitaEX (ours)} & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
			\hline
		\end{tabular}
	\end{table}
...
Concurrently, all data activities follow the \textbf{CRISP-DM} standard, whose six-step cycle—Business Understanding, Data Understanding, Data Preparation, Modeling, Evaluation, and Deployment—provides full traceability from raw web data to production rules.
	...
	\section{Conclusion}
%	VitaEX demonstrates that the fusion of interpretable rule‑based models (Apriori) with state‑of‑the‑art NLP (SBERT embeddings) can substantially enhance the employment readiness of AI students in the Mexican context by converting mined labor‑market regularities into transparent, actionable résumé guidance. From job postings collected between November 2024 and May 2025 the system initially generated 2,075,096 candidate association rules and, after pruning by support (>1\%), confidence (>50\%), lift (≥1.5 where specified) and redundancy elimination, retained 533,889 high‑quality interpretable rules—summarized in the abstract as “over 500,000 meaningful association rules.” These rules surface underlying skill patterns, frequently co‑occurring competencies, and contextual relationships (skills, experience level, location, modality), exemplified by observed linkages such as SQL and Pandas profiles aligning with expectations like TensorFlow expertise. A hybrid scoring function with weighting \(\alpha = 0.6\) toward rule confidence integrates symbolic association strength with semantic similarity, maintaining full transparency via per‑suggestion explanations that build user trust and educational value. The project’s contributions include a localized multi‑city AI vacancy corpus normalized into transactional form; a large, pruned association‑rule knowledge base; a hybrid recommend‑and‑explain mechanism uniting Apriori metrics with SBERT similarity; and a modular full‑stack implementation (React front‑end, Django REST controller with JWT authentication, hybrid MongoDB/MySQL persistence, Celery‑based asynchronous scraping and retraining) that supports iterative refinement. Practical implications span clearer identification of skill gaps for students, empirical co‑occurrence signals for academic stakeholders, and data‑driven regional guidance, while current limitations involve the bounded temporal window, focus on five urban hubs, potential platform selection bias, absence so far of longitudinal placement indicators (e.g., interview callbacks or offers), and the need for periodic re‑mining to address temporal drift and cold‑start emergence of novel technologies.Beyond these quantitative outcomes, the integration of the Spiral Model with CRISP‑DM ensured disciplined iteration: early loops de‑risked scraping scale, rule explosion, and security controls, while each data mining cycle fed directly into refreshed recommendation endpoints. Educational intent remains central—rules are not opaque scores but structured explanations linking a student’s present profile to statistically grounded next steps, aligning with the project goals of empowering graduates, streamlining recruiter evaluation, and promoting regional equity across Mexico City, Guadalajara, Monterrey, Querétaro, and Puebla. The architecture (asynchronous scraping, transactional and semi‑structured persistence separation, REST exposure) supports periodic rule re‑mining without interrupting user interaction, and its modularity enables future incorporation of additional parsing or ranking components consistent with existing interfaces. Future enhancements already outlined—richer keyword/skill extraction (NER), broadened document format ingestion, ontology‑driven normalization of synonymous terms, temporal tracking of emergent skill demand, and incorporation of a feedback loop—are aimed at deepening personalization and resilience against market drift while preserving interpretability. Collectively, VitaEX provides an explainable, region‑aware scaffold that narrows informational gaps for early‑career AI candidates and establishes a reproducible foundation for subsequent refinement cycles anchored in transparent data mining and modern NLP.
	...
	% ----------------- Bibliography -------------------------
\makeatletter
\renewcommand\@biblabel[1]{#1.}  % opcional: cambia [1] por 1.
\renewcommand\bibsection{\section*{References}\vspace*{-0.5em}} % quita salto de pagina 
\makeatother
